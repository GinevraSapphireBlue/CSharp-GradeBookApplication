...
namespace GradeBook.GradeBooks
{
    public abstract class BaseGradeBook
    {
        ...
        {
            var weightedGpaBonus = (IsWeighted && (studentType == StudentType.Honors || studentType == StudentType.DualEnrolled)) ? 1 : 0;
            switch (letterGrade)
            {
                case 'A':
                    return 4 + weightedGpaBonus;
                case 'B':
                    return 3 + weightedGpaBonus;
                case 'C':
                    return 2 + weightedGpaBonus;
                case 'D':
                    return 1 + weightedGpaBonus;
                case 'F':
                    return 0 + weightedGpaBonus;
            }
            return 0;
        }
